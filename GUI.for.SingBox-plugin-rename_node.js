...
async function beautifyNodeName(proxies, metadata) {
  ...
  let flag = Plugins.APP_TITLE.includes('SingBox') ? 'tag' : 'name';
  // 优先根据订阅的 User-Agent 覆盖默认值
  if (typeof metadata?.header?.request === 'object') {
    const ua = metadata.header.request['UserAgent'] ||
               metadata.header.request['User-Agent'] ||
               metadata.header.request['user-agent'];

    if (typeof ua === 'string') {
      const lowerUA = ua.toLowerCase();
      if (lowerUA.includes('clash')) {
        flag = 'name';
      } else if (lowerUA.includes('singbox') || lowerUA.includes('sing-box')) {
        flag = 'tag';
      }
      // 其他情况保持默认值，不需要再写 else
    }
  }
  ...
  proxies = proxies.map((proxy) => {
    ...
    if (typeof tag !== 'string') {
      // 如果没有有效的名称，跳过或给个默认名
      console.warn('Proxy missing name/tag:', proxy)
      return proxy  // 或者 return { ...proxy, [flag]: '未知节点' }
    }
